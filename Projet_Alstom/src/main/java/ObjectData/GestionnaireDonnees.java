...
public class GestionnaireDonnees {
		...
		for (Object o : a) {
			...
			for (Object obj : tabDonnees) {
				...
				if (p.getTypePara() == typeParametre.COMBO) {
					// On s'attend à avoir un autre parametre contenant l'id du
					// select
		...
		for (DataObject data : objet.getDonnees()) {
			...
			if (data.getParametre().getTypePara() == typeParametre.COMBO) {
				document += "\"inputValue\":\"" + data.getValue() + "-"
						+ GC.getParametre((Integer) data.getValue()).getLabel() + "\"}";
			} else {
				document += "\"inputValue\":\"" + data.getValue() + "\"}";
			}
			document += ",";
	...
	public int importerObject(String URL) {
		...
		for (Object o : a) {
			...
			ObjectClass objet = new ObjectClass(Integer.parseInt(job.get("id").toString()),
					job.get("typeObject").toString(), job.get("URL").toString());
			...
			for (Object ob : data) {
				...
				if (da.get("inputType").toString().equals("COMBO")) {
					...
					dataobject = new DataObject(tmp[0],
							GC.getParametre(Integer.parseInt(da.get("inputId").toString())));

				} else {
					dataobject = new DataObject(da.get("inputValue"),
							GC.getParametre(Integer.parseInt(da.get("inputId").toString())));
				}
